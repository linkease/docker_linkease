...
mkdir -p dest
wget -O dest.tar.gz http://fw0.koolcenter.com/binary/LinkEase/LinuxStorage/linkease-binary-${VER}.tar.gz
tar -zxf ./dest.tar.gz -C ./dest/
mv ./dest/linkease-binary-${VER} ./dest/linkease-binary
cp ./linkease-install.sh ./dest/
rm -f ./dest.tar.gz

wget -O media.tar.gz http://fw0.koolcenter.com/binary/LinkEase/LinuxStorage/linkease-media-binary-${MEDIA}.tar.gz
